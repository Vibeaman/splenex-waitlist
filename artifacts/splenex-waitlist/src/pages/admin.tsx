...
function EntryRow({ entry }: { entry: WaitlistEntry }) {
  return <div className="grid grid-cols-[36px_1fr] gap-3 px-4 py-5 sm:grid-cols-[46px_1.1fr_1.35fr_120px_150px] sm:items-center sm:gap-4 sm:px-5" data-testid={`row-entry-${entry.id}`}>
...
export default function Admin() {
  ...
  const exportEntries = () => {
    ...
    const header = 'id,email,twitterUsername,xFollowed,telegramJoined,createdAt';
    const rows = entries.map((entry) => [entry.id, entry.email, entry.twitterUsername, entry.xFollowed, entry.telegramJoined, entry.createdAt].map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(','));
  ...
  return <PageFrame admin>
    <main className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14 lg:px-12">
      ...
      <section className="mt-12">
        ...
        {entriesQuery.isLoading ? <SkeletonRows /> : entries.length ? <div className="border border-border bg-card/20"><div className="hidden grid-cols-[46px_1.1fr_1.35fr_120px_150px] gap-4 border-b border-border px-5 py-3 sm:grid"><span className="mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">ID</span><span className="mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">X identity</span><span className="mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Email</span><span className="mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Checks</span><span className="mono text-right text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Received</span></div><div className="divide-y divide-border">{entries.map((entry) => <EntryRow key={entry.id} entry={entry} />)}</div></div> : <div className="border border-dashed border-border px-6 py-16 text-center" data-testid="status-admin-empty"><div className="mx-auto grid h-10 w-10 place-items-center border border-border text-muted-foreground"><ChevronDown size={16} /></div><p className="mt-5 text-sm text-foreground">{search ? 'No matching members.' : 'The room is still empty.'}</p><p className="mx-auto mt-2 max-w-[35ch] text-xs leading-5 text-muted-foreground">{search ? 'Try a different email or X username.' : 'Once the first signal comes through, members will appear here.'}</p></div>}
      </section>
    </main>
  </PageFrame>;
}